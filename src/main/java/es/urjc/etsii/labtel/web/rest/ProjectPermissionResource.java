...
public class ProjectPermissionResource {
    ...
    /**
     * SEARCH  /_search/project-permissions?query=:query : search for the projectPermission corresponding
     * to the query.
     *
     * @param query the query of the projectPermission search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/project-permissions")
    @Timed
    public ResponseEntity<List<ProjectPermissionDTO>> searchProjectPermissions(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ProjectPermissions for query {}", query);
        Page<ProjectPermissionDTO> page = projectPermissionService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/project-permissions");
